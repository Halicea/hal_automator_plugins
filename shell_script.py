...
class ShellScript(OperationBase):
    ...
    def get_arg_descriptors(cls):
        return [
            ArgumentDescriptor(
                "working directory", "relative or absolute path where the command will be executed", "text"),
            ArgumentDescriptor(
                "command", "shell command that will be executed", "text"),
            ArgumentDescriptor(
                "is sudo", "Whether to execute with sudo privileges", "boolean"),
            ArgumentDescriptor(
                "catch shell output", "Whether the result should be the shell output", "boolean")
        ]
    ...
    def run(self):
        ...
        if os.name != 'nt':
            ...
            p = subprocess.Popen(cmd, stderr=subprocess.PIPE,
                                 stdout=subprocess.PIPE, cwd=self.working_dir, env=os.environ)
        ...
        if code > 0:
            ...
            raise ShellScriptError(
                'Shell command returned an error code different than 0')
