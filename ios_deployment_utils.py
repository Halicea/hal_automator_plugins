...
def add_profile(path, args=None):
    info = get_provistioning_profile_info(path)
    j = os.path.join
...
def rm_profile_like_on_path(path=None, args=None):
    ...
    profile_path = os.path.join(
        profiles_dir, '%s.mobileprovision' % info['uuid'])
    ...
    for root, dirs, files in os.walk(_pdir):
        for f in files:
            if f.endswith('.mobileprovision'):
                ...
                info = get_provistioning_profile_info(pfile)
                if target:
...
def rm_cert(cert_name=None, sha1=None, args=None):
    cmd = None
    if cert_name:
        cmd = ['/usr/bin/security', 'delete-certificate',
               '-c', cert_name, args.keychain]
    if sha1:
        cmd = ['/usr/bin/security', 'delete-certificate',
               '-Z', sha1, args.keychain]
    if cmd:
        print cmd
        call(cmd)
